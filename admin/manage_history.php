...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'added') {
        ...
        $action_message = "<p class='success-message'>校史事件删除成功。</p>";
...
$history_result = $conn->query("SELECT id, event_date, title, description, multimedia_path, created_at, updated_at FROM school_history ORDER BY event_date DESC, created_at DESC");
...
<div class="admin-section-header">
    <a href="add_history.php" class="admin-button">添加新校史事件</a>
</div>
            ...
            <tbody>
                <?php while ($row = $history_result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <a href="edit_history.php?id=<?php echo $row['id']; ?>" class="admin-button-small edit">编辑</a>
                            <a href="delete_history.php?id=<?php echo $row['id']; ?>" class="admin-button-small delete" onclick="return confirm('您确定要删除此校史事件吗？此操作无法撤销。');">删除</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
