...
$title = '';
$content = '';
$current_author_name = ''; // Author name from DB
...
if ($stmt_fetch) {
    ...
    if ($result->num_rows === 1) {
        ...
        $current_author_name = $news_item['author_name'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Author name and token are updated to the current editing teacher from session
    $author_name_from_session = isset($_SESSION['teacher_name']) ? $_SESSION['teacher_name'] : '未知教师';
    $token_used_from_session = isset($_SESSION['teacher_token']) ? $_SESSION['teacher_token'] : null;
    ...
    if ($token_used_from_session === null) {
    ...
    if (empty($error_message)) {
        ...
        if (empty($error_message)) {
            ...
            $stmt_update = $conn->prepare("UPDATE campus_news SET title = ?, content = ?, author_name = ?, token_used = ?, multimedia_path = ? WHERE id = ?");
            if ($stmt_update) {
                $stmt_update->bind_param("sssssi", $title_updated, $content_updated, $author_name_from_session, $token_used_from_session, $new_multimedia_path_for_db, $news_id);
                if ($stmt_update->execute()) {
                    // Log Action
                    $action_taken = "更新新闻文章 ID: " . $news_id . ", 标题: " . $title_updated;
                    $stmt_log = $conn->prepare("INSERT INTO token_logs (token_value, teacher_name_entered, action_taken) VALUES (?, ?, ?)");
                    if($stmt_log){
                        $stmt_log->bind_param("sss", $token_used_from_session, $author_name_from_session, $action_taken);
                        $stmt_log->execute();
                        $stmt_log->close();
                    }
        ...
        // Author name displayed is the original author, but will be updated to current teacher on save
        // $current_author_name remains for display of original author if needed, but form value reflects potential change
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>?id=<?php echo $news_id; ?>" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="author_name_display">原作者:</label>
        <input type="text" id="author_name_display" name="author_name_display" value="<?php echo htmlspecialchars($current_author_name); ?>" readonly disabled>
        <p class="info-text">（注：保存时作者将更新为当前登录教师：<strong><?php echo isset($_SESSION['teacher_name']) ? htmlspecialchars($_SESSION['teacher_name']) : ''; ?></strong>）</p>
    </div>
    ...
    <div class="form-group">
        <input type="submit" value="更新文章" class="teacher-button">
